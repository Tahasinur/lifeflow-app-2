...
class FollowSystemTest {
    ...
    void testUnfollowUser() throws Exception {
        ...
        // Then unfollow
        mockMvc.perform(delete("/api/follows/{followerId}/unfollow/{followingId}", user1.getId(), user2.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success", is(true)))
    ...
    void testIsFollowing() throws Exception {
        ...
        mockMvc.perform(get("/api/follows/{followerId}/is-following/{followingId}", user1.getId(), user2.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.isFollowing", is(true)));

        mockMvc.perform(get("/api/follows/{followerId}/is-following/{followingId}", user2.getId(), user1.getId()))
                .andExpect(status().isOk())
